...
//     noBidPlacedForEvent: 'no bid placed for this event',
//     noBidPlacedForEventOfType: (type: string) => `no ${type} bid was placed for this event`,
//     expectedBidNotHighestPriority: (type: string) => `the expected ${type} bid is not the highest priority bid placed by the flows`,
//     noPendingRequest: 'no pending request found for this action',
//     eventBlocked: (flowId: string) => `event is blocked by flow ${flowId}`,
//     pendingRequest: (flowId: string) => `event has a pending request from flow ${flowId}`,
//     pendingExtend: (flowId: string) => `event has a pending extend. Flow ${flowId} is extending the event.`,
//     invalidPayload: 'the payload is not valid',
//     noAskForBid: 'no askFor bid was placed for this event',
//     externalActionNotBasedOnAskForBid: 'the external action is not based on the highest priority askFor bid',
// }
 ...
 */
export function explainValidation<P, V>(eventInfo: EventInformation<P, V>, value: P, additionalBids: (PlacedBid<P, V> | undefined)[] = []) {
    ...
    const bids = [latestExtendBid, ...additionalBids, ...eventInfo.validate].filter(isDefined);
    if(bids.length !== 0) {
        //TODO: log all event value acccesses
    }
    bids.forEach(bid => {
