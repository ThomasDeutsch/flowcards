...
import { ActionType, ExternalAction, Action } from './action';
...
export class BThread {
    ...
    get currentBids(): BidDictionaries | null {
        if(!(this._currentBidsForProgress[0] === this.nrProgressions) || this._currentBids?.type === BidDictionaryType.function) {
    ...
    public progressWaitIntercept(type: BidType, eventName: string, payload: any): void {
