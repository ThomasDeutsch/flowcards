...
type GetStateFunction = (event: FCEvent | string) => unknown;
...
function stageBThreadsAndEventCaches(
    ...
    const enableEventCache: EnableStateFunctionType = (event: FCEvent | string, initialValue: unknown): StateRef<unknown> => {
        event = toEvent(event);
