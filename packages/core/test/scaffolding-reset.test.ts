...
test("a thread gets reset, when the arguments change", () => {
    ...
    testScenarios((enable) => {
        ...
        enable(threadB({waitingForB: state.bids.askForBid?.has('B')}));
    }, () => {
...
test("a thread gets reset, when the arguments change - 2", () => {
    ...
    testScenarios((enable) => {
        const state = enable(threadA());
        const test: MyProps = state.bids.askForBid?.has('B') ? {waitingForB: state.bids?.askForBid?.has('B')} : {waitingForB: false, waitingForX: false};
        enable(threadB(test));
    }, () => {
        expect(initCount).toBe(2);    
    });
...
test("a thread gets reset, when the arguments change - 3", () => {
    ...
    testScenarios((enable) => {
        const state = enable(threadA());
        const test = state.bids?.askForBid?.has('B') ? {waitingForB: state.bids.askForBid?.has('B')} : undefined;
        enable(threadB(test));
    }, () => {
        expect(initCount).toBe(2);
    });
