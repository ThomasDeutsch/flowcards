...
test("A promise can be requested", () => {
    ...
    scenarios((enable) => {
        ...
        expect(log.latestActionAndReactions.action.eventName).toBe("A");
        expect(log.latestActionAndReactions.action.threadId).toBe("thread1");
        expect(log.latestActionAndReactions.action.type).toBe(ActionType.requested);
    });
});
...
//         expect(last(log.actionsAndReactions).action.type).toBe(ActionType.requested);
//     }));
// });
