...
test("log will contain reactions by thread-id", () => {
    const flow1 = flow(
      ...
      testScenarios((enable) => {
        ...
        expect(log?.bThreadInfoById['flow1'].reactions.get(0)?.bid?.event.name).toEqual('selectProduct');
        expect(flow1Keys[1]).toEqual(1);
        expect(log?.bThreadInfoById['flow1'].reactions.get(1)?.bid?.event.name).toEqual('apiGetProductDetails');
        expect(flow1Keys[2]).toEqual(2);
        expect(log?.bThreadInfoById['flow1'].reactions.get(2)?.bid?.event.name).toEqual('productDetails');
        expect(flow1Keys[3]).toEqual(3);
        expect(log?.bThreadInfoById['flow1'].reactions.get(3)?.bid?.event.name).toEqual('acceptAGB');
        expect(flow1Keys[4]).toEqual(4);
        expect(log?.bThreadInfoById['flow1'].reactions.get(4)?.bid?.event.name).toEqual('agbAccepted');
        // flow 2
        expect(log?.bThreadInfoById['flow2'].reactions.size).toBe(2);
        expect(flow2Keys[0]).toEqual(0);
        expect(log?.bThreadInfoById['flow2'].reactions.get(0)?.bid?.event.name).toEqual('selectProduct');
        expect(log?.bThreadInfoById['flow2'].reactions.get(1)?.bid?.event.name).toBeUndefined();
        expect(log?.bThreadInfoById['flow2'].reactions.get(2)?.bid?.event.name).toBeUndefined();
        expect(flow2Keys[1]).toEqual(3);
        expect(log?.bThreadInfoById['flow2'].reactions.get(3)?.bid?.event.name).toEqual('acceptAGB');
    });
})
