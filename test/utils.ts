...
function getYieldValueFromTests(action: LoggedAction<any> | undefined, useMocks: boolean, remainingTests: ActionAndReactions[], reject: (reason: string) => void, getPendingRequests: () => Placed<RequestBid<any, any>>[]): "mockRequest" | undefined | ExternalAction<any> & {id: number}  | ResolvePendingRequestAction<any> | RejectPendingRequestAction {
...
export function* actionReactionTest(scheduler: Engine, useMocks: boolean, recorded: ActionAndReactions[], remainingTests: ActionAndReactions[], resolve: (nr: number) => void, reject: (reason: string) => void, getPendingRequests: () => Placed<RequestBid<any, any>>[]): ActionReactionGenerator {
...
export async function runFlowcardsTests(testContext: Deno.TestContext, rootFlow: FlowGeneratorFunction, events: EventRecord, tests?: ActionAndReactions[]): Promise<void> {
