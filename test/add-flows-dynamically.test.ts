...
describe("a sub flow can be added/removed dynamically", () => {
    ...
    test('a flow can be added by .flow() from the outside', () => {
        ...
        const scheduler = testSchedulerFactory(function*(this: Flow) {
            yield askFor(eventA);
        });
        // add a flow from outside:
        scheduler.rootFlow.flow('addDynamically', function*() {
    ...
    test('a flow can be removed by .end() from the outside', () => {
        ...
        // add a flow from outside:
        //TODO: expect(scheduler.rootFlow.subFlows.length).toBe(0);
        expect(1).toBe(0);
    });
});